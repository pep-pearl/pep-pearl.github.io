...
- `D:\01_Projects\workspace\lh_dmp`의 폴더 구조 자체는 공개 가능하다. 단, v1에서는 실제 경로/파일명은 인용하지 않고 public-safe 추상 구조 다이어그램으로만 표현한다.
...
- [x] 블로그 링크 `https://jinjoo648.tistory.com/` 공개 여부를 최종 확인했는가?
...
- [x] GitHub Pages는 user site `https://pep-pearl.github.io/`로 배포하는가?
- [x] repository 이름 기준 project site로 배포한다면 Vite `base`는 무엇이어야 하는가?
- [ ] React Router 없이 원페이지 anchor navigation으로 v1을 진행할 것인가?
- [x] Framer Motion을 사용할 만큼의 모션 요구가 있는가, CSS transition만으로 충분한가?
...
- [x] Case 01 구현 단계의 `lh_dmp` 폴더 구조 공개 정책은 확정되었다. 구조 자체는 공개 가능하지만, v1에서는 실제 경로/파일명 인용 없이 public-safe 추상 구조 다이어그램으로만 표현한다.
